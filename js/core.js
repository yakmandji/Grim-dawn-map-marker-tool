...
function getUserDataOnly() {
  ...
  // Save current map
  function saveLastActiveProfile() {
    try {
      if (state.active) {
        localStorage.setItem('gdmm_last_active_profile', state.active);
      }
    } catch (e) {
      console.warn('Cannot persist last active profile', e);
    }
  }
