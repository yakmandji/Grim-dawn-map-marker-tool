...
// END RENDER MARKER ---------------------------------------------------------
  ...
  function centerOn(xp, yp, targetScale = 1.5, markerId = null) {
    ...
    const scale = clamp(targetScale || state.view.scale, 0.30, 1.50);
...
/*END------------------------------------------*/
  ...
  //ZOOM FONCTION
    function zoomAt(clientX, clientY, step) {
      ...
      const ns = clamp(old * (1 + step), 0.30, 1.50);
      if (ns === old) return;
      const vb = viewport.getBoundingClientRect();
  ...
  const MAP_SOURCES = {
    ...
    'Asterkarn':    'https://www.grimcustommarker.org/maps/asterkarn_profile.json?v=1.10',
  };
