...
const {
  ...
  mapImg.addEventListener('load', () => {
    ...
    // Si on a une vue sauvegardée pour ce profil, on la réutilise
  ...
  let viewPersistenceEnabled = false;
  let viewSaveTimer = null;

  // Save current zoom
  function persistCurrentView() {
    if (!viewPersistenceEnabled) return;
    const p = currentProfile();
    if (!p) return;
    p.view = { ...state.view };
...
function renderMarkers(options = {}) {
  ...
  // === rifts/region selection ===
  let riftData = [];
  (function () {
    if (!state.active || !window.MAP_SETS) return;
    const name = state.active.toLowerCase();

    for (const key in window.MAP_SETS) {
      if (name.includes(key)) {
        riftData = window.MAP_SETS[key].rifts || [];
        return;
      }
    }
  })();
  ...
  riftData.forEach(m => {
    ...
    // position via pctToPx (comme les autres marqueurs)
    const pt = pctToPx(m.xp ?? m.x, m.yp ?? m.y);
    el.style.left = pt.x + 'px';
    el.style.top  = pt.y + 'px';
  ...
  (function () {
    ...
    for (const key in window.MAP_SETS) {
      if (name.includes(key)) {
        regionData = window.MAP_SETS[key].regions || [];
        return;
      }
    }
  })();
...
//---------------------------------------------------------------------------------------
  ...
  // --- Init on load ---
  (async () => {
    const REMOTE_JSON_URL = 'https://yakmandji.github.io/Grim-dawn-map-marker-tool/gdmm_all_profiles.json?v=3.4';
    // empty base
    ...
    // Restaure last map showed
    try {
      const last = localStorage.getItem('gdmm_last_active_profile');
